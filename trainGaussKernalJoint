function sessionID= trainGaussKernalJoint(dbTrain, dbVal, varargin)
...
for iEpoch= 1:opts.nEpoch
    ...
    for iBatch= 1:nBatches
        ...
        for iQuery= 1:opts.batchSize
            ...
            dsSq=  exp(2*feats(:, 1)'*bsxfun(@minus, feats(:, 3:end), feats(:, 2)));
            ...
            for nv = 3:thisNumIms
                ...
                dzdy(:, 1) = dzdy(:, 1) + 2/(1+sum(dsSq))*dsSq(nv-2)*(feats(:,nv)-feats(:,2));
                
                dzdy(:, 2) = dzdy(:, 2) + 2/(1+sum(dsSq))*dsSq(nv-2)*(-feats(:,1));
                
                dzdy(:, nv) = dzdy(:, nv) + 2/(1+sum(dsSq))*dsSq(nv-2)*feats(:,1);
