function sessionID= trainAngularLoss(dbTrain, dbVal, varargin)
...
for iEpoch= 1:opts.nEpoch
    ...
    for iBatch= 1:nBatches
        ...
        for iQuery= 1:opts.batchSize
            ...
            fapn = zeros(1,thisNumIms-2);
            indexs = [];
            
            nViolatingNegs = 0;
            for ki = 1:thisNumIms-2

                fapn(ki) = 4*tan(angular)^2*(feats(:, 1) + feats(:, 2))'*feats(:, ki+2) - 2*(1+tan(angular)^2)*feats(:, 1)'*feats(:, 2);
                loss = loss + exp(fapn(ki));
                indexs = [indexs ki];
                
                nViolatingNegs = nViolatingNegs + 1;
            end
            ...
            for nv = 3:thisNumIms
                ...
                dzdy(:, 1) = dzdy(:, 1) + exp(fapn(nv-2))/(1+sum(exp(fapn(indexs))))*(4*tan(angular)^2*feats(:,nv)-2*(1+tan(angular)^2)*feats(:,2));
                dzdy(:, 2) = dzdy(:, 2) + exp(fapn(nv-2))/(1+sum(exp(fapn(indexs))))*(4*tan(angular)^2*feats(:,nv)-2*(1+tan(angular)^2)*feats(:,1));
                dzdy(:, nv) = dzdy(:, nv) + exp(fapn(nv-2))/(1+sum(exp(fapn(indexs))))*(4*tan(angular)^2*(feats(:,1) + feats(:,2)));
