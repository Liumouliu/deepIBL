function sessionID= trainExpKernalInd(dbTrain, dbVal, varargin)
opts= struct(...
    'netID', 'caffe', ...
    'layerName', 'conv5', ...
    ...
    for iBatch= 1:nBatches
        ...
        for iQuery= 1:opts.batchSize
            ...
            for ki = 1:thisNumIms-2
                cpq(ki) = dsSq_t(1)/(dsSq_t(1) + dsSq_t(1+ki));
                loss = loss - log(cpq(ki));
            end
            ...
            for nv = 3:thisNumIms
                ...
                dzdp_t = -(1.0-cpq(nv-2))/dsSq(1)*(feats(:,1)-feats(:,2));
                dzdn_t = (1.0-cpq(nv-2))/dsSq(nv-1)*(feats(:,1)-feats(:,nv));
