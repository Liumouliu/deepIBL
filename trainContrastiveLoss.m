function sessionID= trainContrastiveLoss(dbTrain, dbVal, varargin)
opts= struct(...
    'netID', 'caffe', ...
    'layerName', 'conv5', ...
...
% the dim of netvlad extracted feature
dimNetVLAD = relja_netOutputDim(net);
...
for iEpoch= 1:opts.nEpoch
    ...
    for iBatch= 1:nBatches
        ...
        for iQuery= 1:opts.batchSize
            ...
            dsSq= sqrt(sum( bsxfun(@minus, feats(:, 1), feats(:, 2:end)) .^2, 1 )');
            
            losses(end+1)= 0.5*dsSq(1)^2 + sum(0.5.*max(0,0.7-dsSq(2:end)).^2);
            
            violatingNegs= dsSq(2:end) <= 0.7;
            ...
            % dzdq  query
            dzdy(:,1) = feats(:,1)-feats(:,2);
            
            % dzdp  positive
            dzdy(:,2) = feats(:,2)-feats(:,1);
            
            violatingNegAbsInds= 2+find(violatingNegs);
            
            for nv = 1:nViolatingNegs
                
                temp_ind = violatingNegAbsInds(nv);
                % query
                dzdy(:, 1) = dzdy(:, 1) + (1-0.7/dsSq(temp_ind-1))*(feats(:,1)-feats(:,temp_ind));
                % negative
                dzdy(:, temp_ind) = dzdy(:, temp_ind) - (1-0.7/dsSq(temp_ind-1))*(feats(:,1)-feats(:,temp_ind));
                
            end
            
            numTriplets= [numTriplets, nViolatingNegs + 1 ]; % add 1 for query-postive pair
