function sessionID= trainCauchyKernelJoint(dbTrain, dbVal, varargin)
opts= struct(...
    'netID', 'caffe', ...
    'layerName', 'conv5', ...
...
for iEpoch= 1:opts.nEpoch
    ...
    for iBatch= 1:nBatches
        ...
        for iQuery= 1:opts.batchSize
            ...
            res= vl_simplenn(net, ims, [], [], 'mode', 'normal', 'conserveMemory', true); 
            ...
            for nv = 3:thisNumIms
                ...
                dzdp_t = 2.0/(1.0+sum(cpq))*(feats(:,2)-feats(:,1))/(1+dsSq(nv-1));
                dzdn_t = 2.0/(1.0+sum(cpq))*(feats(:,1)-feats(:,nv))*(1+dPos)/(1+dsSq(nv-1))^2;
