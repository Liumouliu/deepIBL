function sessionID= trainLiftStruct(dbTrain, dbVal, varargin)
...
for iEpoch= 1:opts.nEpoch
    ...
    for iBatch= 1:nBatches
        ...
        for iQuery= 1:opts.batchSize
            ...
            dzdy(:, 1) = Jij/dsSq(1)*(feats(:,1)-feats(:,2));
            ...
            for nv = 3:thisNumIms
                ...
                dtemp = Jij/cpq*exp(alpha - dsSq(nv-1))*(feats(:,1)-feats(:,nv))/dsSq(nv-1);

                dzdy(:, 1) = dzdy(:, 1) - dtemp;
                
                dzdy(:, nv) = dzdy(:, nv) + dtemp;
