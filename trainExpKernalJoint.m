function sessionID= trainExpKernalJoint(dbTrain, dbVal, varargin)
...
for iEpoch= 1:opts.nEpoch
    ...
    for iBatch= 1:nBatches
        ...
        for iQuery= 1:opts.batchSize
            ...
            dsSq_diff = exp(dsSq(1)-dsSq(2:end));
                       
            loss = log(1+sum(dsSq_diff));
            losses(end+1)= loss;
            ...
            for nv = 3:thisNumIms
                ...
                dzdp_t = -1.0/(1.0+sum(dsSq_diff))/dsSq(1)*dsSq_diff(nv-2)*(feats(:,1)-feats(:,2));
                dzdn_t = 1.0/(1.0+sum(dsSq_diff))/dsSq(nv-1)*dsSq_diff(nv-2)*(feats(:,1)-feats(:,nv));
                
                
                dzdy(:, 1) = dzdy(:, 1) - dzdp_t - dzdn_t;
                
                dzdy(:, 2) = dzdy(:, 2) + dzdp_t;
                
                dzdy(:, nv) = dzdy(:, nv) + dzdn_t;
